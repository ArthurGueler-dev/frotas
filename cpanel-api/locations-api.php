...
function handleGet($pdo, $id) {
    if ($id) {
        ...
        // Aceitar tanto blockId quanto block_id
        if ((isset($_GET['blockId']) && !empty($_GET['blockId'])) || (isset($_GET['block_id']) && !empty($_GET['block_id']))) {
            $where[] = "l.block_id = ?";
            $blockIdParam = isset($_GET['blockId']) ? $_GET['blockId'] : $_GET['block_id'];
            $params[] = intval($blockIdParam);
        }
