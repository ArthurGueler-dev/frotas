<?php
/**
 * Registrar Peças Suspensão/Outros da Strada na Tabela de Compatibilidade
 * USA O NOME CORRETO: "STRADA 1.4 Endurance"
 *
 * Executar: https://floripa.in9automacao.com.br/registrar-compatibilidade-strada-suspensao.php?confirmar=SIM
 */
...
$categoriaMap = [
    ...
    'iluminacao' => 'Sistema Elétrico',
    'injecao' => 'Motor'
];

// Buscar peças de suspensão/outros criadas hoje
$stmt = $pdo->prepare("
    SELECT id, codigo, nome, categoria, custo_unitario
    FROM FF_Pecas
    WHERE nome LIKE '%STRADA%'
      AND categoria IN ('suspensao', 'ar_condicionado', 'iluminacao', 'injecao')
      AND ativo = 1
...
if (count($pecas) === 0) {
    $stmt = $pdo->prepare("
        ...
        WHERE nome LIKE '%STRADA%'
          AND (categoria IN ('suspensao', 'ar_condicionado', 'iluminacao', 'injecao')
               OR nome LIKE '%AMORT%' OR nome LIKE '%BIELETA%' OR nome LIKE '%BANDEJA%'
               OR nome LIKE '%BALANCA%' OR nome LIKE '%BARRA AXIAL%')
          AND ativo = 1
        ORDER BY categoria, nome
    ");
    $stmt->execute();
...
if (count($pecas) === 0) {
    echo json_encode([
        'success' => false,
        'error' => 'Nenhuma peça de suspensão da Strada encontrada',
        'sugestao' => 'Execute primeiro: importar-pecas-strada-suspensao.php'
    ], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    exit;
}
...
try {
    foreach ($pecas as $peca) {
        ...
        $obs = "Peça Suspensão/Outros importada via Perplexity AI - " . date('d/m/Y');
