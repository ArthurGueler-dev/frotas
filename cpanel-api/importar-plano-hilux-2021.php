<?php
/**
 * Script INTELIGENTE para importar Plano de Manutenção Toyota HILUX 2021
 * Busca automaticamente o modelo correto no banco e importa os itens
 *
 * Acesso: https://floripa.in9automacao.com.br/importar-plano-hilux-2021.php?confirmar=SIM
 */
...
if (!isset($_GET['confirmar']) || $_GET['confirmar'] !== 'SIM') {
    ...
    echo json_encode([
        ...
        'url_exemplo' => 'https://floripa.in9automacao.com.br/importar-plano-hilux-2021.php?confirmar=SIM'
    ], JSON_PRETTY_PRINT);
    exit();
}
...
try {
    // PASSO 1: Buscar modelo Hilux no banco
    $stmt = $conn->prepare("
        SELECT id, marca, modelo, ano
        FROM FF_VehicleModels
        WHERE (modelo LIKE '%HILUX%' OR marca LIKE '%Toyota%')
    ...
    if ($result->num_rows === 0) {
        echo json_encode([
            'success' => false,
            'error' => 'Modelo HILUX 2021 não encontrado no banco',
            'instrucao' => 'Cadastre primeiro o modelo na página de modelos',
            'exemplo' => 'Marca: Toyota, Modelo: HILUX CD SRX, Ano: 2021'
        ], JSON_PRETTY_PRINT);
        exit();
    }
    ...
    // Nome do modelo que será usado no campo modelo_carro
    $modeloNome = $modelo['modelo']; // Usar APENAS o campo modelo (como a API busca)

    // PASSO 2: Deletar planos antigos deste modelo (se existirem)
    $stmt = $conn->prepare("DELETE FROM Planos_Manutenção WHERE modelo_carro LIKE ?");
    $like = "%{$modeloNome}%";
    ...
    // PASSO 3: Inserir todos os itens do plano
    $itens_plano = [
        // 1.000 km
    ...
    echo json_encode([
        ...
        'modelo_usado_sql' => $modeloNome,
        'itens_deletados' => $deletados,
        'itens_inseridos' => $inseridos,
        'total_itens' => count($itens_plano),
        'erros' => $erros,
        'proximo_passo' => 'Acesse https://frotas.in9automacao.com.br/planos-manutencao.html?modeloId=' . $modelo['id']
