        ...
        if (autoClustering && insertedIds.length > 0) {
            ...
            for (const block of pythonBlocks) {
                ...
                for (const block of pythonBlocks) {
                    ...
                    try {
                        console.log('💾 Salvando bloco:', block.name, 'com', block.locationIds?.length || 0, 'locais');

                        const payload = {
                            createSingleBlock: true,
                            name: block.name,
                        ...
                        if (saveResponse.ok) {
                            ...
                            console.log('✅ Bloco salvo:', saveData);
                            if (saveData.success && saveData.block) {
                                block.id = saveData.block.id;
                            } else {
                                console.error('❌ Salvamento falhou:', saveData);
                            }
                        } else {
                            const errorText = await saveResponse.text();
                            console.error('❌ Erro HTTP ao salvar bloco:', saveResponse.status, errorText);
                        }
                    } catch (err) {
                        console.error('❌ ERRO CRÍTICO ao salvar bloco:', err);
